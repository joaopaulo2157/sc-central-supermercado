...
function getBootstrap({ admin = false } = {}) {
  ...
  const productSql = admin ? 'SELECT * FROM products ORDER BY id' : 'SELECT * FROM products WHERE active=1 ORDER BY id';
...
function calculateOrder(payload) {
  ...
  for (const raw of items) {
    ...
    const qty = Math.max(1, Math.min(99, Number(raw.quantity ?? raw.qty) || 1));
  ...
  let region = null;
  let deliveryFee = 0;
  let minimum = Number(getSettingsObject().minimumOrder || 0);

  if (method === 'Entrega') {
    const regionId = Number(payload.regionId || 0);
    const regionName = safeText(payload.regionName || payload.neighborhood || '', 120);
    region = regionId
      ? db.prepare('SELECT * FROM delivery_regions WHERE id=? AND active=1').get(regionId)
      : db.prepare('SELECT * FROM delivery_regions WHERE name=? AND active=1').get(regionName);
    ...
    if ((subtotal - discount) < minimum) {
      throw Object.assign(new Error(`Pedido mínimo para ${region.name}: R$ ${minimum.toFixed(2)}.`), { statusCode: 400 });
    }
  }
...
function createOrder(payload) {
  ...
  try {
    // Revalida estoque dentro da transação e faz a reserva do pedido.
    for (const item of calc.canonicalItems) {
      const row = db.prepare('SELECT stock FROM products WHERE id=?').get(item.productId);
      if (!row || Number(row.stock) < item.qty) {
        throw Object.assign(new Error(`Estoque alterado para ${item.name}. Atualize o carrinho.`), { statusCode: 409 });
      }
    }

    db.prepare(`INSERT INTO orders(
      id,created_at,updated_at,customer_name,phone,method,region_id,region_name,address,reference,
      delivery_time,payment,change_for,notes,coupon_code,subtotal,discount,delivery_fee,total,status
    ) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
      ...
      safeText(payload.address || '', 250),
      safeText(payload.reference || '', 180),
      safeText(payload.deliveryTime || '', 80),
      safeText(payload.paymentMethod || payload.payment || 'A combinar', 80),
      safeText(payload.changeFor || '', 80),
      safeText(payload.notes || '', 1000),
      calc.couponCode,
      calc.subtotal, calc.discount, calc.deliveryFee, calc.total,
      'novo'
    );

    const itemStmt = db.prepare(`INSERT INTO order_items(order_id,product_id,name,unit,price,qty) VALUES(?,?,?,?,?,?)`);
    const stockStmt = db.prepare('UPDATE products SET stock=stock-?, updated_at=? WHERE id=?');
    for (const item of calc.canonicalItems) {
      itemStmt.run(id, item.productId, item.name, item.unit, item.price, item.qty);
    ...
    db.prepare(`INSERT INTO order_events(order_id,status,note,user_id,created_at) VALUES(?,?,?,?,?)`)
      .run(id, 'novo', 'Pedido criado pelo site e preparado para envio ao WhatsApp.', null, timestamp);
    bumpVersion();
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
  ...
  return getOrder(id, { includePrivate:true });
}

function getOrder(id, { includePrivate = false } = {}) {
  const row = db.prepare('SELECT * FROM orders WHERE id=?').get(id);
  if (!row) return null;
  const items = db.prepare('SELECT product_id,name,unit,price,qty FROM order_items WHERE order_id=? ORDER BY id').all(id).map(i => ({
    productId: Number(i.product_id), name:i.name, unit:i.unit, price:Number(i.price), qty:Number(i.qty)
  }));
  const events = db.prepare(`SELECT e.status,e.note,e.created_at,u.name AS user_name
                             FROM order_events e LEFT JOIN users u ON u.id=e.user_id
                             WHERE e.order_id=? ORDER BY e.id`).all(id).map(e => ({
    status:e.status, note:e.note, createdAt:e.created_at, userName:e.user_name || ''
  }));
  const base = {
    id: row.id, createdAt: row.created_at, updatedAt: row.updated_at,
    customer: row.customer_name, method: row.method, regionName: row.region_name,
    payment: row.payment, couponCode: row.coupon_code,
    subtotal:Number(row.subtotal), discount:Number(row.discount), deliveryFee:Number(row.delivery_fee), total:Number(row.total),
    status: row.status, items, events
  };
  if (includePrivate) {
    Object.assign(base, {
      phone: row.phone, address: row.address, reference: row.reference,
      deliveryTime: row.delivery_time, changeFor: row.change_for, notes: row.notes,
...
function updateOrderStatus(orderIdValue, newStatus, note, user) {
  ...
  return getOrder(orderIdValue, { includePrivate:true });
}
...
function whatsappMessage(order, settings) {
  ...
  const lines = order.items.map((item, index) => `${index+1}. ${item.qty}x ${item.name} (${item.unit}) — ${money(item.price * item.qty)}`);
  return [
    ...
    order.method === 'Entrega' ? `🏘️ *Região:* ${order.regionName}` : '',
    order.method === 'Entrega' ? `📍 *Endereço:* ${order.address || '-'}` : '',
    order.method === 'Entrega' ? `🧭 *Referência:* ${order.reference || '-'}` : '',
    order.method === 'Entrega' ? `🕐 *Horário:* ${order.deliveryTime || 'O quanto antes'}` : '',
    `💳 *Pagamento:* ${order.payment || 'A combinar'}`,
    order.payment === 'Dinheiro' && order.changeFor ? `💵 *Troco para:* ${order.changeFor}` : '',
...
async function handleApi(req, res, url) {
  ...
  if (method === 'GET' && pathname === '/api/health') {
    return sendJson(res, 200, { ok:true, version:'5.0.0', database:path.basename(DB_PATH), changeVersion:Number(getMeta('change_version','1')), time:now() });
  }
  ...
  if (method === 'POST' && pathname === '/api/orders') {
    ...
    if (!/^\d{12,13}$/.test(storeWhatsapp)) {
      return sendJson(res, 409, { ok:false, error:'O WhatsApp da loja ainda não foi configurado no Painel V5.' });
    }
    const body = await readJson(req);
  ...
  if (pathname.startsWith('/api/admin/')) {
    ...
    if (method === 'POST' && pathname === '/api/admin/products') {
      ...
      const result=db.prepare(`INSERT INTO products(name,category,unit,price,old_price,badge,emoji,stock,featured,image,active,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)`).run(
    ...
    if(method==='PUT'&&pathname==='/api/admin/settings'){
      if(user.role!=='admin')return sendJson(res,403,{ok:false,error:'Apenas administrador pode alterar configurações gerais.'});const b=await readJson(req);const allowed=['storeName','whatsapp','cartGoal','minimumOrder','primaryMessage','openingHours','address','allowDelivery','allowPickup'];const current=getSettingsObject();for(const key of allowed)if(Object.prototype.hasOwnProperty.call(b,key))current[key]=b[key];setSettingsObject(current);audit(user.id,'update','settings','store');return sendJson(res,200,{ok:true,settings:current});
    }
...
const server = http.createServer(async (req, res) => {
  ...
  try {
    ...
    console.error('[SC CENTRAL V5]', error);
...
server.listen(PORT, HOST, () => {
  console.log('');
  console.log('╔══════════════════════════════════════════════════════╗');
  console.log('║        SUPERMERCADO SC CENTRAL • V5 ATIVA          ║');
  console.log('╚══════════════════════════════════════════════════════╝');
