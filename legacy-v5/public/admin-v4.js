// ==========================================================
// PAINEL ADMINISTRATIVO V4 - SUPERMERCADO SC CENTRAL
...
(() => {
  ...
  const state = {
    ...
    activeTab:'dashboard',
    bootstrap:null,
    products:[], banners:[], coupons:{}, regions:[], settings:{},
  ...
  function switchTab(tab) {
    ...
    const names={dashboard:'Dashboard',products:'Produtos',banners:'Banners',coupons:'Cupons',delivery:'Entrega',orders:'Pedidos',users:'Usuários',audit:'Auditoria',settings:'Configurações'};
  ...
  async function renderDashboard() {
    ...
    e.healthGrid.innerHTML=`
      <div class="health-item"><div><strong>Servidor V5</strong><small>${esc(health.time)}</small></div><b>ONLINE</b></div>
  ...
  function productCategories() { return [...new Set(state.products.map(p=>p.category).filter(Boolean))].sort(); }
  ...
  function renderProducts() {
    ...
    const term=(e.productSearch.value||'').toLowerCase();
    const cat=e.productCategoryFilter.value;
    const status=e.productStatusFilter.value;
    const list=state.products.filter(p=>{
      const search=!term||p.name.toLowerCase().includes(term)||p.category.toLowerCase().includes(term);
      const category=cat==='todos'||p.category===cat;
      const st=status==='todos'||(status==='ativos'&&p.active)||(status==='sem-estoque'&&Number(p.stock)<=0)||(status==='baixo'&&Number(p.stock)>0&&Number(p.stock)<=8);
      return search&&category&&st;
    });
    e.productsTableBody.innerHTML=list.map(p=>`<tr>
      <td><div class="product-cell"><div class="product-thumb">${productThumb(p)}</div><div><strong>${esc(p.name)}</strong><small>${esc(p.unit)} • ${esc(p.badge||'')}</small></div></div></td>
      <td>${esc(p.category)}</td><td><strong>${money(p.price)}</strong>${p.oldPrice?`<br><small><s>${money(p.oldPrice)}</s></small>`:''}</td>
      <td><span class="status-pill ${Number(p.stock)<=0?'off':Number(p.stock)<=8?'low':''}">${Number(p.stock)} un.</span></td>
      <td><span class="status-pill ${p.active?'':'inactive'}">${p.active?'Ativo':'Inativo'}</span></td>
      <td>${p.featured?'⭐ Sim':'—'}</td>
      <td><div class="table-actions"><button class="icon-btn" data-edit-product="${p.id}">✎</button><button class="icon-btn danger" data-delete-product="${p.id}">×</button></div></td>
    </tr>`).join('') || '<tr><td colspan="7">Nenhum produto encontrado.</td></tr>';
  }

  function productForm(p={}) {
    state.imageData=p.image||'';
    return `<form class="modal-form-v4" id="productForm">
      <label class="full"><span>Nome do produto</span><input name="name" required value="${esc(p.name||'')}"></label>
      <label><span>Categoria</span><input name="category" required value="${esc(p.category||'mercearia')}"></label>
      <label><span>Unidade / apresentação</span><input name="unit" value="${esc(p.unit||'')}"></label>
      ...
      <label><span>Estoque</span><input name="stock" type="number" min="0" step="1" value="${p.stock??0}"></label>
      <label><span>Badge</span><input name="badge" value="${esc(p.badge||'')}"></label>
      <label><span>Emoji</span><input name="emoji" value="${esc(p.emoji||'🛒')}"></label>
      <label class="full"><span>URL da imagem</span><input name="image" id="productImageUrl" value="${esc(p.image||'')}" placeholder="https://..."></label>
      <div class="image-tools-v4"><button type="button" id="chooseImageBtn">Selecionar imagem do computador</button><button type="button" id="clearImageBtn">Remover imagem</button></div>
  ...
  function bindProductForm(id=null) {
    ...
    e.imageFile.onchange=()=>{
      const file=e.imageFile.files?.[0]; if(!file)return;
      if(file.size>1.5*1024*1024){toast('Imagem muito grande. Use até 1,5 MB.');e.imageFile.value='';return;}
      const reader=new FileReader();reader.onload=()=>{state.imageData=String(reader.result);imageUrl.value='';preview.innerHTML=`<img src="${state.imageData}" alt="">`;};reader.readAsDataURL(file);
    };
    form.addEventListener('submit',async ev=>{
      ev.preventDefault(); const fd=new FormData(form);
      const payload={name:fd.get('name'),category:fd.get('category'),unit:fd.get('unit'),price:Number(fd.get('price')||0),oldPrice:fd.get('oldPrice')===''?null:Number(fd.get('oldPrice')),stock:Number(fd.get('stock')||0),badge:fd.get('badge'),emoji:fd.get('emoji'),image:state.imageData||String(fd.get('image')||''),featured:fd.get('featured')==='on',active:fd.get('active')==='on'};
      await api(id?`/api/admin/products/${id}`:'/api/admin/products',{method:id?'PUT':'POST',body:JSON.stringify(payload)});
      toast(id?'Produto atualizado.':'Produto cadastrado.');closeModal();await loadBootstrap();renderProducts();renderDashboard();
    });
  }
