...
(() => {
  ...
  function productImageMarkup(product, className = "") {
    if (product.image) {
      return `<img class="${className}" src="${product.image}" alt="${product.name}" loading="lazy" onerror="this.remove()">`;
    }
  ...
  function updateCheckoutReviewV3() {
    ...
    const final = totals.total + ctx.fee;
    ...
    if (v3el.checkoutDeliveryFee) v3el.checkoutDeliveryFee.textContent = ctx.method === "Retirada na loja" ? "Grátis" : (ctx.neighborhood ? money(ctx.fee) : "A calcular");
  ...
  const submitCheckoutV3 = event => {
    ...
    if (!whatsappIsConfigured()) return showToast("⚠️", "Configure o WhatsApp no Painel V3 ou no script.");
    ...
    const finalTotal = totals.total + ctx.fee;
