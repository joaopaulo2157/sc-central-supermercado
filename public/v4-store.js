// ==========================================================
// SC CENTRAL V6 FINAL - INTEGRAÇÃO DA LOJA COM API
...
(() => {
  ...
  function serverUnavailableNotice() {
    ...
    box.innerHTML = `<span>⚠️ V6 em modo local. Para usar banco de dados, usuários e sincronização, abra o projeto com <b>npm start</b>.</span><button type="button">Fechar</button>`;
  ...
  function buildOrderPayload() {
    ...
    return {
      ...
      substitutionPreference: String(formData.get('substitutionPreference') || 'contact'),
  ...
  checkoutFormNode?.addEventListener('submit', async event => {
    ...
    try {
      ...
      if (!/^\d{12,13}$/.test(phone)) {
        throw new Error(`Pedido ${result.order.id} foi salvo, mas o WhatsApp da loja ainda não foi configurado no Painel V6.`);
      }
